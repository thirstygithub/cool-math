...
require "engine/text"
require "challenges"
require "engine/threadreplace"


-- game.js index.html read from

require "js" -- https://github.com/Davidobot/love.js/issues/89  -- https://github.com/MrcSnm/Love.js-Api-Player
-- require "engine/sound_manager"
...
function love.run()
	...
	return function()
		...
		xpcall(function()
			...
			print("[Lua Error in love.update]:", err)
			print(debug.traceback())
		  end)


		G.FPS_CAP = G.FPS_CAP or (G.F_MOBILE and 30 or 100)
...
function love.load()
	...
	G:init_window()
	if love.platform and love.platform.earlyInit then
		love.platform.earlyInit()
	end

	if love.platform and love.platform.isArcade and love.platform.isArcade() then
...
function love.update( dt )
	...
	if delay > 0.5 then
	  print("[Warning] Frame delayed!", delay)
	end
	lastTime = now
end
