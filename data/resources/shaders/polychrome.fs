...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	MY_HIGHP_OR_MEDIUMP number saturation_fac = 1. - max(0., 0.05*(1.1-delta));

	MY_HIGHP_OR_MEDIUMP vec4 hsl = HSL(vec4(tex.r*saturation_fac, tex.g*saturation_fac, tex.b, tex.a));

	MY_HIGHP_OR_MEDIUMP float t = polychrome.y*2.221 + time;
	MY_HIGHP_OR_MEDIUMP vec2 floored_uv = (floor((uv*texture_details.ba)))/texture_details.ba;
    MY_HIGHP_OR_MEDIUMP vec2 uv_scaled_centered = (floored_uv - 0.5) * 50.;
    ...
    MY_HIGHP_OR_MEDIUMP float res = (.5 + .5* cos( (polychrome.x) * 2.612 + ( field + -.5 ) *3.14));
