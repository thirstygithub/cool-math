...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    MY_HIGHP_OR_MEDIUMP vec4 hsl = HSL(0.5*tex + 0.5*vec4(0.,0.,1.,tex.a));

	MY_HIGHP_OR_MEDIUMP float t = holo.y*7.221 + time;
	MY_HIGHP_OR_MEDIUMP vec2 floored_uv = (floor((uv*texture_details.ba)))/texture_details.ba;
    MY_HIGHP_OR_MEDIUMP vec2 uv_scaled_centered = (floored_uv - 0.5) * 250.;
	...
	MY_HIGHP_OR_MEDIUMP float res = (.5 + .5* cos( (holo.x) * 2.612 + ( field + -.5 ) *3.14));

	MY_HIGHP_OR_MEDIUMP number low = min(tex.r, min(tex.g, tex.b));
    MY_HIGHP_OR_MEDIUMP number high = max(tex.r, max(tex.g, tex.b));
	MY_HIGHP_OR_MEDIUMP number delta = 0.2+0.3*(high- low) + 0.1*high;

	MY_HIGHP_OR_MEDIUMP number gridsize = 0.79;
    MY_HIGHP_OR_MEDIUMP number fac = 0.5*max(max(max(0., 7.*abs(cos(uv.x*gridsize*20.))-6.),max(0., 7.*cos(uv.y*gridsize*45. + uv.x*gridsize*20.)-6.)), max(0., 7.*cos(uv.y*gridsize*45. - uv.x*gridsize*20.)-6.));
