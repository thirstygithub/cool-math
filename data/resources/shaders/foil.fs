...
{
	MY_HIGHP_OR_MEDIUMP number hs = mod(h, 1.)*6.;
...
vec4 RGB(vec4 c)
{
	...
	MY_HIGHP_OR_MEDIUMP number t = (c.z < .5) ? c.y*c.z + c.z : -c.y*c.z + (c.y+c.z);
	MY_HIGHP_OR_MEDIUMP number s = 2.0 * c.z - t;
	return vec4(hue(s,t,c.x + 1./3.), hue(s,t,c.x), hue(s,t,c.x - 1./3.), c.w);
}

vec4 HSL(vec4 c)
{
	MY_HIGHP_OR_MEDIUMP number low = min(c.r, min(c.g, c.b));
	MY_HIGHP_OR_MEDIUMP number high = max(c.r, max(c.g, c.b));
	MY_HIGHP_OR_MEDIUMP number delta = high - low;
	MY_HIGHP_OR_MEDIUMP number sum = high+low;

	MY_HIGHP_OR_MEDIUMP vec4 hsl = vec4(.0, .0, .5 * sum, c.a);
	if (delta == .0)
		return hsl;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    MY_HIGHP_OR_MEDIUMP vec2 adjusted_uv = uv - vec2(0.5, 0.5);
	...
	MY_HIGHP_OR_MEDIUMP number delta = min(high, max(0.5, 1. - low));

    MY_HIGHP_OR_MEDIUMP number fac = max(min(2.*sin((length(90.*adjusted_uv) + foil.r*2.) + 3.*(1.+0.8*cos(length(113.1121*adjusted_uv) - foil.r*3.121))) - 1. - max(5.-length(90.*adjusted_uv), 0.), 1.), 0.);
    MY_HIGHP_OR_MEDIUMP vec2 rotater = vec2(cos(foil.r*0.1221), sin(foil.r*0.3512));
    MY_HIGHP_OR_MEDIUMP number angle = dot(rotater, adjusted_uv)/(length(rotater)*length(adjusted_uv));
    MY_HIGHP_OR_MEDIUMP number fac2 = max(min(5.*cos(foil.g*0.3 + angle*3.14*(2.2+0.9*sin(foil.r*1.65 + 0.2*foil.g))) - 4. - max(2.-length(20.*adjusted_uv), 0.), 1.), 0.);
    MY_HIGHP_OR_MEDIUMP number fac3 = 0.3*max(min(2.*sin(foil.r*5. + uv.x*3. + 3.*(1.+0.5*cos(foil.r*7.))) - 1., 1.), -1.);
    MY_HIGHP_OR_MEDIUMP number fac4 = 0.3*max(min(2.*sin(foil.r*6.66 + uv.y*3.8 + 3.*(1.+0.5*cos(foil.r*3.414))) - 1., 1.), -1.);

    MY_HIGHP_OR_MEDIUMP number maxfac = max(max(fac, max(fac2, max(fac3, max(fac4, 0.0)))) + 2.2*(fac+fac2+fac3+fac4), 0.);
