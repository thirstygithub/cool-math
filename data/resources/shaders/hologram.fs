...
{
    //Glow effect
    MY_HIGHP_OR_MEDIUMP number glow = 0.;
    MY_HIGHP_OR_MEDIUMP int glow_samples = 4;
    MY_HIGHP_OR_MEDIUMP int actual_glow_samples = 0;
    MY_HIGHP_OR_MEDIUMP number glow_dist = 0.0015;
    MY_HIGHP_OR_MEDIUMP number _a = 0.;
    ...
    MY_HIGHP_OR_MEDIUMP number timefac = 1.0*hologram.g;
    ...
    MY_HIGHP_OR_MEDIUMP number light_strength = 0.4*(0.3*sin(2.*hologram.g) + 0.6 + 0.3*sin(hologram.r*3.) + 0.9);
    MY_HIGHP_OR_MEDIUMP vec4 final_col;
    if (tex.a < 0.001){
